...
- [ ] Repair or explicitly isolate the existing root test configuration failures before adding new suites.
- [ ] Record an architecture decision for the companion technology after a small metadata-capture and encrypted-storage spike.
- [ ] Confirm local-only inference, retention defaults, smoothing threshold, rounding, and unassigned release behavior.
- [ ] Define a synthetic labeled-day fixture format and metric calculations.
- [ ] Capture the existing manual time-entry API contract as regression tests.
...
- [ ] Add backward-compatible core time-entry fields and tenant migration.
- [ ] Add lock enforcement and idempotent batch creation.
- [ ] Add typed time-entry and issue-candidate capabilities to the SDK.
- [ ] Add capability checks to the server plugin context.
- [ ] Add manifest validation, `requires`, and companion metadata.
- [ ] Make install and upgrade migration handling atomic and fail closed.
- [ ] Add core unit and HTTP boundary tests.
...
- A dedicated application directory selected in Phase 0
...
Resolve these in Phase 0 or during the smallest relevant spike:

- Native Swift versus another companion runtime
- Local inference runtime and model
- Exact raw-signal retention and post-release deletion timing
- Browser metadata mechanism and permission UX
- Rounding policy for official minutes
- Whether an unassigned draft may ever be released without an issue
