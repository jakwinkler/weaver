...
- [x] **Hook EventDispatcher into WebSocket gateway** -- EventDispatcherService now injects WeaverGateway and calls `emitToTenant()` / `emitToProject()` when domain events fire. _File: `apps/api/src/modules/events/event-dispatcher.service.ts`_
...
- [x] **userId included in all event payloads** -- `issue.created`, `issue.updated`, `issue.assigned`, `issue.moved`, `issue.status_changed`, `issue.deleted` all include `userId`. _File: `apps/api/src/modules/issues/issues.service.ts`_
- [x] **Comment events** -- `comment.created` and `comment.deleted` events emitted from CommentsService. CommentsModule now imports EventsModule. _Files: `apps/api/src/modules/comments/comments.service.ts`, `comments.module.ts`_
- [x] **Tenant-scoped channels** -- Events emitted to tenant room (`tenant:{tenantId}`) and optionally project room (`project:{projectKey}`) based on tenant context from AsyncLocalStorage. _File: `event-dispatcher.service.ts`_

### Frontend

- [x] **Created useWebSocket hook** -- `apps/web/src/hooks/useWebSocket.ts`. Connects to Socket.IO `/ws` namespace using JWT from auth store. Auto-reconnects with exponential backoff. Listens for all domain events.
- [x] **Invalidate React Query on events** -- Maps domain events to query key invalidations (issues, comments, activity, boards, dashboard, projects).
...
- [x] **Toast notifications** -- Shows lightweight toast notifications for events from OTHER users (compares `userId` from event payload to current user). Auto-dismisses after 4 seconds. No external toast library needed.
...
- [x] **E2E: Tenant isolation** -- Events from tenant A not received by tenant B's socket. _File: `apps/api/test/realtime.e2e-spec.ts`_

## Acceptance Criteria

- [x] Board updates live when another user moves a card (query invalidation on issue.moved/status_changed)
- [x] New comments appear without refresh (query invalidation on comment.created)
...
- EventDispatcherService reads tenant context from AsyncLocalStorage to determine which tenant room to emit to
- Frontend connects once in AppLayout; all domain events trigger React Query invalidation
- Toast system is CSS-only (no external dependency) with auto-dismiss
