...
export class RateLimitingGuard implements CanActivate, OnModuleInit, OnModuleDestroy {
  ...
  constructor(
    ...
    @Optional() @InjectRepository(ApiKeyEntity)
  ...
  private async verifyAccessCredential(credential?: string): Promise<string | undefined> {
    ...
    if (credential.startsWith(API_KEY_PREFIX)) {
      const key = await this.apiKeyRepo?.findOneBy({ keyHash: createHash('sha256').update(credential).digest('hex') });
      return key && (!key.expiresAt || key.expiresAt.getTime() > Date.now()) ? `api-key:${key.id}` : undefined;
    }
